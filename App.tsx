...
const App = () => {
  ...
  // --- ESTADO CENTRAL DE DADOS (MINEPAD) ---
  const [meetings, setMeetings] = useState<any[]>([
    { 
      id: 1, title: 'Reunião do Conselho - Planejamento Estratégico', status: 'Agendada', date: '2026-03-15', time: '14:00', type: 'Híbrida', link: 'https://zoom.us/inepad',
  ...
  const [auditLogs, setAuditLogs] = useState<any[]>([
    { id: 1, date: '19/02/2026 14:30', user: 'Sistema', action: 'Update', details: 'Correções críticas e sincronização de status aplicadas.' }
  ]);
  ...
  // --- FUNÇÃO GLOBAL DE ALTERAÇÃO DE STATUS ---
  const updateActionStatus = (meetingId: number, actionId: number, newStatus: string) => {
    ...
    // Se estiver editando a reunião atual, atualiza o estado local também
    ...
    addLog('Status', `Ação #${actionId} alterada para ${newStatus}.`);
  };
      ...
      <div className="animate-in fade-in pb-20">
        ...
        {tab === 'pauta' && (
          <div className="bg-white p-8 rounded-[40px] border shadow-sm animate-in fade-in space-y-6"><h3 className="text-[10px] font-black uppercase italic text-slate-500">Ordem do Dia</h3><div className="space-y-3">{(currentMeeting.pautas || []).map((p:any, i:any) => (<div key={i} className="flex gap-4 p-5 border rounded-3xl bg-slate-50/50 text-xs items-center shadow-sm font-bold italic"><GripVertical className="text-slate-200"/><div className="flex-1">{p.title}</div><div className="font-black text-slate-400 uppercase text-[9px]">{p.resp}</div><div className="font-black text-blue-600">{p.dur} MIN</div><button onClick={()=>setCurrentMeeting({...currentMeeting, pautas: currentMeeting.pautas.filter((_:any, idx:any)=>idx!==i)})}><Trash2 size={16}/></button></div>))}</div><div className="p-8 bg-slate-50 rounded-[32px] border-2 border-dashed grid grid-cols-4 gap-3">
              ...
              <button onClick={()=>{if(tmpPauta.title){setCurrentMeeting({...currentMeeting, pautas:[...(currentMeeting.pautas || []), tmpPauta]}); setTmpPauta({title:'', resp:'', dur:''});}}} className="col-span-4 bg-blue-600 text-white rounded-2xl py-3 font-black text-[10px] uppercase">Adicionar à Pauta</button></div></div>
        )}
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-16 bg-white border-b flex items-center justify-between px-8">
          <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest italic">INEPAD Ribeirão Preto</span>
          <div className="flex gap-4 items-center">
            <div className="text-right"><p className="text-xs font-black text-slate-800 uppercase italic">{currentUser.name}</p><p className="text-[9px] font-bold text-blue-600 uppercase tracking-widest">{currentUser.role}</p></div>
