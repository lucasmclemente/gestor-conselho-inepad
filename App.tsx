...
const App = () => {
  ...
  const [auditLogs, setAuditLogs] = useState<any[]>([
    { id: 1, date: '19/02/2026 13:45', user: 'Sistema', action: 'Estabilidade', details: 'Versão blindada aplicada para evitar falhas de renderização.' }
  ]);
  ...
  const [meetings, setMeetings] = useState<any[]>([
    { 
      id: 1, title: 'Conselho Admin - INEPAD Q4', status: 'CONCLUÍDA', date: '2025-12-10', time: '10:00', type: 'Presencial', address: 'Sede Inepad',
      participants: [{name: 'Membro Teste', email: 'teste@inepad.com.br'}],
      pautas: [{title: 'Aprovação de Verba Marketing', dur: '20', resp: 'Admin'}],
      materiais: [], deliberacoes: [], acoes: [{id: 201, title: 'Branding', status: 'Concluído'}], atas: []
    }
  ]);
  ...
  // --- CÁLCULOS DO DASHBOARD (COM PROTEÇÃO) ---
  const stats = useMemo(() => {
    ...
    const atrasado = pendingList.filter((a:any) => a.date && new Date(a.date) < today).length;
    const pendente = Math.max(0, pendingList.length - atrasado);

    const pieData = [
      { name: 'OK', value: concluido || 0, color: '#10b981' },
      { name: 'Pendente', value: pendente || 0, color: '#3b82f6' },
      { name: 'Atrasado', value: atrasado || 0, color: '#ef4444' }
    ];

    const barData = meetings.map(m => ({
      name: m.title?.substring(0, 10) || 'Reunião',
        ...
        <div className="w-full max-w-md bg-white rounded-[40px] shadow-2xl overflow-hidden">
          ...
          <div className="p-10">
            <form onSubmit={(e) => {
              ...
              <button type="submit" className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black text-xs uppercase shadow-xl hover:bg-blue-700 transition-all">Acessar Sistema</button>
            </form>
          </div>
        </div>
      </div>
  ...
  if (activeMenu === 'dashboard') {
      ...
      <div className="space-y-8 animate-in fade-in">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-white p-8 rounded-[40px] border shadow-sm h-96">
            ...
            <div className="h-64"><ResponsiveContainer width="100%" height="100%"><BarChart data={stats.barData}><CartesianGrid vertical={false} stroke="#f1f5f9" /><XAxis dataKey="name" tick={{fontSize: 9}} /><YAxis hide /><Tooltip/><Legend iconType="circle" wrapperStyle={{fontSize:'10px', fontWeight:900}} /><Bar dataKey="Pautas" fill="#cbd5e1" radius={[6,6,0,0]} barSize={20}/><Bar dataKey="Ações" fill="#3b82f6" radius={[6,6,0,0]} barSize={20}/></BarChart></ResponsiveContainer></div>
          </div>
          <div className="bg-white p-8 rounded-[40px] border shadow-sm h-96 flex flex-col">
            <h3 className="text-[10px] font-black uppercase mb-6 italic text-slate-500">Status das Ações</h3>
        ...
        <div className="bg-white p-8 rounded-[40px] border shadow-sm flex justify-between items-center">
            <h3 className="text-[10px] font-black uppercase italic text-slate-500 flex items-center gap-2"><Target size={16}/> Plano de Ações</h3>
            <button onClick={() => setActiveMenu('plano-geral')} className="text-blue-600 text-[10px] font-black uppercase hover:underline font-bold">Ver Todas</button>
        </div>
      </div>
    );
    ...
    mainContent = (
      <div className="space-y-6 animate-in fade-in">
        ...
        <div className="bg-white p-8 rounded-[40px] border shadow-lg overflow-hidden"><h2 className="text-xl font-black italic mb-6">Monitoramento Geral</h2><table className="w-full text-left font-bold italic text-xs"><thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400 border-b"><tr><th className="p-6">Tarefa</th><th className="p-6">Origem</th><th className="p-6">Status</th></tr></thead><tbody>{stats.pendingList.map((a:any, i:number) => (<tr key={i} className="border-t hover:bg-slate-50/50"><td className="p-6 text-slate-800 font-black italic underline">{a.title}</td><td className="p-6 text-slate-400 uppercase font-black">{a.mTitle}</td><td className="p-6"><span className="bg-blue-100 px-4 py-1.5 rounded-full text-[8px] font-black uppercase">{a.status}</span></td></tr>))}</tbody></table></div>
      </div>
    );
  } else if (activeMenu === 'reunioes') {
    mainContent = view === 'list' ? (
      <div className="space-y-6 animate-in fade-in">
        <div className="flex justify-between items-center"><h1 className="text-2xl font-black italic tracking-tighter">Histórico</h1><button onClick={()=>{setCurrentMeeting({id:0, title:'', status:'AGENDADA', date:'', time:'', type:'Híbrida', participants:[], pautas:[], materiais:[], deliberacoes:[], acoes:[], atas:[]}); setView('details'); setTab('info');}} className="bg-blue-600 text-white px-6 py-2.5 rounded-2xl font-black text-xs uppercase shadow-xl hover:scale-105">+ Agendar</button></div>
