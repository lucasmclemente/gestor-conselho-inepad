...
const App = () => {
  ...
  // --- ESTADO CENTRAL DA GESTÃO INEPAD ---
  ...
  const handleUpload = (e: any, target: 'materiais' | 'atas') => {
    ...
    if(f) {
      const entry = { id: Date.now(), name: f.name, uploadedBy: 'Ricardo Oliveira', date: new Date().toLocaleString('pt-PT') };
    ...
    <div className="min-h-screen bg-[#f8fafc] flex font-sans text-slate-900 overflow-hidden leading-tight">
      ...
      <aside className="w-64 bg-[#0f172a] text-slate-300 flex flex-col shadow-2xl shrink-0">
        ...
        <nav className="flex-1 p-4 space-y-2">
          <button onClick={() => {setActiveMenu('dashboard'); setView('list');}} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold ${activeMenu === 'dashboard' ? 'bg-blue-600 text-white' : 'hover:bg-slate-800'}`}><LayoutDashboard size={18}/> Dashboard</button>
          <button onClick={() => {setActiveMenu('reunioes'); setView('list');}} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold ${activeMenu === 'reunioes' ? 'bg-blue-600 text-white' : 'hover:bg-slate-800'}`}><Calendar size={18}/> Reuniões</button>
        </nav>
      </aside>
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-[#f8fafc]">
          {view === 'list' ? (
            <div className="space-y-6">
              <div className="flex justify-between items-center"><h1 className="text-2xl font-black italic">Histórico de Reuniões</h1><button onClick={() => setView('details')} className="bg-blue-600 text-white px-5 py-2 rounded-xl font-bold">+ Nova</button></div>
            ...
            <div className="animate-in fade-in pb-20">
              ...
              {/* ABA 1: INFORMAÇÕES (LOGÍSTICA HÍBRIDA) */}
              {tab === 'info' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in">
                  <div className="bg-white p-6 rounded-3xl border shadow-sm">
                    ...
                    <div className="space-y-2 mb-4">{meeting.participants.map((p, i) => (<div key={i} className="flex justify-between p-3 bg-slate-50 rounded-xl text-xs font-bold italic border"><div>{p.name}<p className="text-[9px] text-slate-400 font-medium tracking-tight">{p.email}</p></div><button onClick={() => setMeeting({...meeting, participants: meeting.participants.filter((_, idx) => idx !== i)})}><X size={14} className="text-slate-200 hover:text-red-500"/></button></div>))}</div>
                    <div className="space-y-2 p-3 bg-slate-50 rounded-xl border-2 border-dashed border-slate-200"><input placeholder="Nome" className="w-full p-2 text-xs border rounded outline-none" value={newPart.name} onChange={e => setNewPart({...newPart, name: e.target.value})} /><input placeholder="Email" className="w-full p-2 text-xs border rounded outline-none" value={newPart.email} onChange={e => setNewPart({...newPart, email: e.target.value})} /><button onClick={() => {if(newPart.name){setMeeting({...meeting, participants:[...meeting.participants, newPart]}); setNewPart({name:'', email:''});}}} className="w-full py-2 bg-blue-600 text-white rounded text-xs font-bold">Adicionar</button></div>
                  </div>
                  <div className="bg-white p-6 rounded-3xl border shadow-sm space-y-4">
                    <h3 className="text-xs font-bold uppercase mb-4 flex items-center gap-2 italic"><Clock size={16}/> Logística</h3>
                    <div className="flex gap-2">{['Online', 'Presencial', 'Híbrida'].map(t => (<button key={t} onClick={() => setMeeting({...meeting, type: t})} className={`flex-1 py-1.5 border rounded text-[10px] font-bold ${meeting.type === t ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-50 text-slate-400'}`}>{t}</button>))}</div>
              ...
              {tab === 'pauta' && (
                <div className="bg-white p-8 rounded-3xl border shadow-sm animate-in fade-in">
                  <div className="flex justify-between items-center mb-6"><h3 className="text-xs font-bold uppercase tracking-widest italic">Pautas Agendadas</h3><span className="text-xs text-blue-600 font-bold uppercase italic">Tempo: {meeting.pautas.reduce((acc, p) => acc + Number(p.duration || 0), 0)} min</span></div>
                  ...
                  <div className="p-6 bg-slate-50 rounded-2xl border-2 border-dashed grid grid-cols-4 gap-2"><input placeholder="Assunto" className="col-span-2 p-2 border rounded text-xs outline-none" value={newPauta.title} onChange={e => setNewPauta({...newPauta, title: e.target.value})} /><select className="p-2 border rounded text-xs outline-none" value={newPauta.responsible} onChange={e => setNewPauta({...newPauta, responsible: e.target.value})}><option value="">Dono</option>{meeting.participants.map((p, i) => <option key={i} value={p.name}>{p.name}</option>)}</select><input type="number" placeholder="Min" className="p-2 border rounded text-xs outline-none" value={newPauta.duration} onChange={e => setNewPauta({...newPauta, duration: e.target.value})} /><button onClick={() => {if(newPauta.title){setMeeting({...meeting, pautas:[...meeting.pautas, newPauta]}); setNewPauta({title:'', responsible:'', duration:''});}}} className="col-span-4 bg-blue-600 text-white rounded-xl py-2 font-bold text-[10px] uppercase">Adicionar Pauta</button></div>
                </div>
              )}
              ...
              {tab === 'materiais' && (
                <div className="bg-white p-8 rounded-3xl border shadow-sm animate-in fade-in">
                  <div className="flex justify-between mb-8 border-b pb-4"><h3 className="font-bold text-xs uppercase tracking-widest italic">Documentos de Apoio</h3><button onClick={() => fileRef.current?.click()} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase shadow-lg shadow-blue-600/20 transition-all hover:bg-blue-700"><Upload size={14} className="inline mr-2"/>Subir Material</button><input type="file" ref={fileRef} className="hidden" onChange={(e) => handleUpload(e, 'materiais')} /></div>
                  <div className="grid grid-cols-2 gap-4">{meeting.materiais.map(m => (<div key={m.id} className="p-4 bg-slate-50 rounded-2xl flex items-center gap-4 border group shadow-sm transition-all hover:border-blue-200"><FileText size={24} className="text-blue-500"/><div className="flex-1 text-xs font-bold truncate italic">{m.name}<p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{m.uploadedBy} • {m.date}</p></div><button onClick={() => setMeeting({...meeting, materiais: meeting.materiais.filter(x => x.id !== m.id)})} className="opacity-0 group-hover:opacity-100 text-red-300 transition-all"><Trash2 size={16}/></button></div>))}</div>
                </div>
              )}

              {/* ABA 4: DELIBERAÇÕES (COM VÍNCULO) */}
              {tab === 'delib' && (
                <div className="space-y-6 animate-in fade-in">
                  <div className="bg-white p-6 rounded-3xl border shadow-sm space-y-4">
                    <h3 className="text-xs font-bold uppercase italic tracking-widest text-slate-400">Configurar Votação</h3>
                    ...
                    <div className="flex flex-wrap gap-2 text-[10px] font-bold text-slate-400 uppercase">Documentos: {meeting.materiais.map(m => (<button key={m.id} onClick={() => setNewDelib({...newDelib, mats: newDelib.mats.includes(m.name)?newDelib.mats.filter(x=>x!==m.name):[...newDelib.mats, m.name]})} className={`px-2 py-1 rounded border ${newDelib.mats.includes(m.name)?'bg-slate-800 text-white shadow-sm':'bg-white'}`}>{m.name}</button>))}</div>
                    ...
                    <button onClick={() => {if(newDelib.title && newDelib.voters.length > 0){setMeeting({...meeting, deliberacoes:[...meeting.deliberacoes, {id: Date.now(), title: newDelib.title, mats: newDelib.mats, votes: newDelib.voters.map(n=>({name:n, status:'Pendente', just:'', saved:false}))}]}); setNewDelib({title:'', mats:[], voters:[]});}}} className="bg-blue-600 text-white px-6 py-2 rounded-xl text-xs font-bold uppercase shadow-lg shadow-blue-600/20">Habilitar Votação</button>
                  </div>
                  {meeting.deliberacoes.map(d => (
                    <div key={d.id} className="bg-white rounded-3xl border overflow-hidden shadow-sm">
                      <div className="p-5 bg-slate-50 border-b flex justify-between items-center"><span className="font-black italic text-slate-700">{d.title}</span><div className="flex gap-1">{d.mats.map((m:any, i:any)=><span key={i} className="text-[9px] font-bold bg-blue-100 text-blue-600 px-2 py-0.5 rounded border border-blue-200 uppercase tracking-tighter">#{m}</span>)}</div></div>
                      <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                        {d.votes.map((v:any, idx:number) => (
                          <div key={idx} className={`p-5 rounded-2xl border-2 ${v.saved ? 'bg-slate-50 opacity-75' : 'bg-white shadow-sm'}`}>
                            ...
                            {v.status.includes('Ressalva') && <textarea disabled={v.saved} className="w-full p-2 bg-amber-50 border border-amber-100 rounded text-[10px] mb-2 font-medium outline-none" placeholder="Justificativa da ressalva..." value={v.just} onChange={e => {const up = meeting.deliberacoes.map(x => x.id === d.id ? {...x, votes: x.votes.map((v2:any) => v2.name === v.name ? {...v2, just: e.target.value} : v2)} : x); setMeeting({...meeting, deliberacoes: up});}} />}
                            {!v.saved && v.status !== 'Pendente' && <button onClick={() => {const up = meeting.deliberacoes.map(x => x.id === d.id ? {...x, votes: x.votes.map((v2:any) => v2.name === v.name ? {...v2, saved: true} : v2)} : x); setMeeting({...meeting, deliberacoes: up});}} className="w-full py-2 bg-slate-800 text-white rounded-xl text-[9px] font-black uppercase shadow-lg">Confirmar e Gravar Voto</button>}
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              {tab === 'acoes' && (
                <div className="space-y-6 animate-in fade-in">
                  <div className="bg-white p-8 rounded-3xl border shadow-sm space-y-4">
                    <h3 className="text-xs font-bold uppercase flex items-center gap-2 italic"><Target size={18} className="text-blue-600" /> Nova Ação Corretiva</h3>
                    ...
                    <button onClick={() => {if(newAcao.title){setMeeting({...meeting, acoes: [...meeting.acoes, {...newAcao, id: Date.now()}]}); setNewAcao({title:'', resp:'', date:'', status:'Pendente'});}}} className="w-full py-3 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase shadow-lg shadow-blue-600/20 transition-all hover:bg-blue-700">Adicionar Tarefa</button>
                  </div>
                  <div className="bg-white rounded-3xl border shadow-sm overflow-hidden">
                    <table className="w-full text-left text-xs"><thead className="bg-slate-50 font-black uppercase text-slate-400 border-b"><tr><th className="p-6">Tarefa</th><th className="p-6">Dono</th><th className="p-6">Prazo</th><th className="p-6">Status</th></tr></thead>
                      <tbody>{meeting.acoes.map(a => (<tr key={a.id} className="border-t hover:bg-slate-50/50"><td className="p-6 font-bold text-slate-700 italic">{a.title}</td><td className="p-6 font-medium text-slate-500">{a.resp}</td><td className="p-6 font-bold text-slate-400">{a.date}</td><td className="p-6"><select value={a.status} onChange={e => setMeeting({...meeting, acoes: meeting.acoes.map(x => x.id === a.id ? {...x, status: e.target.value} : x)})} className="p-2 border rounded-lg font-black uppercase text-[9px] bg-white shadow-sm">{['Pendente', 'Andamento', 'Concluído'].map(s => <option key={s} value={s}>{s}</option>)}</select></td></tr>))}</tbody>
                    </table>
                  </div>
                </div>
              )}
              ...
              {tab === 'atas' && (
                <div className="bg-white p-8 rounded-3xl border shadow-sm animate-in fade-in">
                  <div className="flex justify-between items-center mb-8 border-b pb-4">
                    ...
                    <button onClick={() => ataRef.current?.click()} className="bg-green-600 text-white px-5 py-2 rounded-xl text-[10px] font-bold uppercase shadow-lg shadow-green-600/20 hover:bg-green-700"><Upload size={14} className="inline mr-2"/>Subir Ata</button><input type="file" ref={ataRef} className="hidden" onChange={(e) => handleUpload(e, 'atas')} />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    {meeting.atas.length === 0 && <div className="col-span-full py-20 text-center border-2 border-dashed border-slate-100 rounded-3xl text-slate-300 font-black uppercase text-xs">Nenhuma Ata Finalizada.</div>}
                  </div>
                </div>
              )}
            </div>
