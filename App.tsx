        ...
        <div className="flex-1 overflow-y-auto p-8">
          {loading ? (
            ...
            <>
                ...
                view === 'list' ? (
                  ...
                  <div className="animate-in fade-in pb-20">
                    ...
                    <div className="border-b flex gap-6 mb-8 overflow-x-auto font-black text-[9px] uppercase italic tracking-widest">
                      {['Informações', 'Ordem do Dia', 'Materiais', 'Deliberações', 'Plano de Ação', 'Atas'].map((t, i) => (
                    ...
                    {/* CONTEÚDO DAS ABAS DAS REUNIÕES (RESTAURADO) */}
                    {tab === 'info' && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in">
                        <div className="bg-white p-6 rounded-[32px] border shadow-sm"><h3 className="text-[10px] font-black uppercase mb-4 italic text-slate-400 tracking-widest">Participantes</h3><div className="space-y-2 mb-4">{(currentMeeting.participants || []).map((p:any, i:any) => (<div key={i} className="flex justify-between p-3 bg-slate-50 rounded-2xl text-[10px] font-bold italic"><span>{p.name}</span><button onClick={()=>setCurrentMeeting({...currentMeeting, participants: currentMeeting.participants.filter((_:any, idx:any)=>idx!==i)})}><X size={14}/></button></div>))}</div><div className="flex gap-2"><input placeholder="Nome" className="flex-1 p-2 text-xs border rounded-xl" value={tmpPart.name} onChange={e=>setTmpPart({...tmpPart, name:e.target.value})} /><button onClick={()=>{if(tmpPart.name){setCurrentMeeting({...currentMeeting, participants:[...(currentMeeting.participants || []), tmpPart]}); setTmpPart({name:'', email:''});}}} className="px-4 py-2 bg-blue-600 text-white rounded-xl text-[9px] font-black uppercase">Add</button></div></div>
                        <div className="bg-white p-6 rounded-[32px] border shadow-sm space-y-4 h-fit"><h3 className="text-[10px] font-black uppercase mb-4 italic text-slate-400 tracking-widest">Logística</h3><div className="flex gap-2">{['Online', 'Presencial', 'Híbrida'].map(t => (<button key={t} onClick={()=>setCurrentMeeting({...currentMeeting, type: t})} className={`flex-1 py-3 border rounded-xl text-[9px] font-black uppercase transition-all ${currentMeeting.type === t ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-50 text-slate-400'}`}>{t}</button>))}</div><div className="grid grid-cols-2 gap-2"><input type="date" value={currentMeeting.date} className="p-3 border rounded-xl text-xs font-bold" onChange={e=>setCurrentMeeting({...currentMeeting, date:e.target.value})} /><input type="time" value={currentMeeting.time} className="p-3 border rounded-xl text-xs font-bold" onChange={e=>setCurrentMeeting({...currentMeeting, time:e.target.value})} /></div></div>
                      </div>
                    )}

                    {tab === 'pauta' && (
                      <div className="bg-white p-8 rounded-[40px] border shadow-sm animate-in fade-in space-y-6">
                        <div className="space-y-3">{(currentMeeting.pautas || []).map((p:any, i:any) => (<div key={i} className="flex justify-between p-4 bg-slate-50 rounded-2xl border-l-4 border-blue-500 font-bold italic text-xs"><span>{p.title} <span className="text-slate-400 ml-2">({p.dur} min)</span></span><button onClick={()=>setCurrentMeeting({...currentMeeting, pautas: currentMeeting.pautas.filter((_:any, idx:any)=>idx!==i)})}><X size={16}/></button></div>))}</div>
                        <div className="grid grid-cols-3 gap-3 p-4 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200"><input placeholder="Assunto" className="p-2 border rounded-xl text-xs" value={tmpPauta.title} onChange={e=>setTmpPauta({...tmpPauta, title:e.target.value})} /><input placeholder="Minutos" className="p-2 border rounded-xl text-xs" type="number" value={tmpPauta.dur} onChange={e=>setTmpPauta({...tmpPauta, dur:e.target.value})} /><button onClick={()=>{if(tmpPauta.title){setCurrentMeeting({...currentMeeting, pautas:[...(currentMeeting.pautas || []), tmpPauta]}); setTmpPauta({title:'', resp:'', dur:''});}}} className="bg-blue-600 text-white rounded-xl text-[9px] font-black uppercase">Adicionar</button></div>
                      </div>
                    )}
                    ...
                    {tab === 'delib' && (
                      <div className="bg-white p-8 rounded-[40px] border shadow-sm animate-in fade-in space-y-6">
                        <div className="space-y-4">{(currentMeeting.deliberacoes || []).map((d:any, i:any) => (<div key={i} className="p-5 bg-amber-50 rounded-3xl border border-amber-100 font-bold italic text-xs flex justify-between"><div><p className="text-amber-800 mb-1">Deliberação #{i+1}</p><p className="text-slate-700">{d.title}</p></div><button onClick={()=>setCurrentMeeting({...currentMeeting, deliberacoes: currentMeeting.deliberacoes.filter((_:any, idx:any)=>idx!==i)})}><X size={18}/></button></div>))}</div>
                        <div className="p-6 bg-slate-50 rounded-[32px] border-2 border-dashed border-slate-200 space-y-3"><textarea placeholder="Descreva a deliberação..." className="w-full p-4 border rounded-2xl text-xs h-24 italic" value={tmpDelib.title} onChange={e=>setTmpDelib({...tmpDelib, title:e.target.value})} /><button onClick={()=>{if(tmpDelib.title){setCurrentMeeting({...currentMeeting, deliberacoes:[...(currentMeeting.deliberacoes || []), tmpDelib]}); setTmpDelib({title:'', voters:[]});}}} className="w-full py-3 bg-amber-500 text-white rounded-xl text-[9px] font-black uppercase shadow-md">Registrar Deliberação</button></div>
                      </div>
                    )}

                    {tab === 'acoes' && (
                      <div className="bg-white p-8 rounded-[40px] border shadow-sm animate-in fade-in space-y-6">
                        <div className="space-y-3">{(currentMeeting.acoes || []).map((a:any, i:any) => (<div key={i} className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100 flex justify-between items-center text-xs font-bold italic"><div><span className="text-emerald-700">{a.title}</span><p className="text-[10px] text-slate-400 italic">Responsável: {a.resp} • Prazo: {a.date}</p></div><button onClick={()=>setCurrentMeeting({...currentMeeting, acoes: currentMeeting.acoes.filter((_:any, idx:any)=>idx!==i)})}><Trash2 size={16} className="text-red-300"/></button></div>))}</div>
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 p-5 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200"><input placeholder="Ação" className="p-2 border rounded-xl text-xs" value={tmpAcao.title} onChange={e=>setTmpAcao({...tmpAcao, title:e.target.value})} /><input placeholder="Responsável" className="p-2 border rounded-xl text-xs" value={tmpAcao.resp} onChange={e=>setTmpAcao({...tmpAcao, resp:e.target.value})} /><input type="date" className="p-2 border rounded-xl text-xs" value={tmpAcao.date} onChange={e=>setTmpAcao({...tmpAcao, date:e.target.value})} /><button onClick={()=>{if(tmpAcao.title){setCurrentMeeting({...currentMeeting, acoes:[...(currentMeeting.acoes || []), {...tmpAcao, id: Date.now()}]}); setTmpAcao({title:'', resp:'', date:'', status:'Pendente'});}}} className="bg-emerald-600 text-white rounded-xl text-[9px] font-black uppercase">Criar Ação</button></div>
                      </div>
                    )}

                    {tab === 'atas' && (
                      <div className="bg-white p-8 rounded-[40px] border shadow-sm animate-in fade-in space-y-8">
                        <div className="flex justify-between items-center border-b pb-4"><h3 className="text-[10px] font-black uppercase flex items-center gap-2 italic text-green-700 tracking-widest"><FileCheck size={18}/> Atas Finais</h3><button onClick={()=>ataRef.current?.click()} className="bg-green-600 text-white px-6 py-3 rounded-2xl text-[9px] font-black uppercase shadow-xl hover:scale-105 transition-all"><Upload size={14} className="inline mr-2"/>Subir Ata Oficial</button></div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {(currentMeeting.atas || []).map((ata:any, i:any) => (
                            <div key={i} className="p-5 bg-green-50 border border-green-100 rounded-[32px] flex items-center gap-4 group shadow-sm">
                              <FileCheck size={28} className="text-green-600"/><div className="flex-1 min-w-0 font-bold italic"><p className="text-sm text-slate-700 truncate">{ata.name}</p></div>
                              <a href={ata.url} target="_blank" rel="noreferrer" className="p-2 text-green-600"><ExternalLink size={18}/></a>
                              <button onClick={()=>setCurrentMeeting({...currentMeeting, atas: currentMeeting.atas.filter((_:any, idx:any) => idx !== i)})} className="text-red-300 hover:text-red-500"><Trash2 size={18}/></button>
                            </div>
                          ))}
                        </div>
                      </div>
              ...
              {activeMenu === 'usuarios' && (
                <div className="space-y-8 animate-in fade-in">
                  <h1 className="text-2xl font-black italic text-slate-800 tracking-tighter">Gestão de Membros</h1>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="bg-white p-6 rounded-[32px] border shadow-sm space-y-4 h-fit sticky top-0">
                      <h3 className="text-[10px] font-black uppercase italic text-slate-400 tracking-widest">Novo Perfil</h3>
                      <input placeholder="Nome" className="w-full p-3 bg-slate-50 border rounded-xl text-xs font-bold outline-none" value={newUserForm.name} onChange={e=>setNewUserForm({...newUserForm, name: e.target.value})} />
                      <input placeholder="E-mail" className="w-full p-3 bg-slate-50 border rounded-xl text-xs font-bold outline-none" value={newUserForm.email} onChange={e=>setNewUserForm({...newUserForm, email: e.target.value})} />
                      ...
                      <select className="w-full p-3 border rounded-xl text-[10px] font-black uppercase italic" value={newUserForm.role} onChange={e=>setNewUserForm({...newUserForm, role: e.target.value})}>
              ...
              {activeMenu === 'auditoria' && (
                <div className="space-y-6 animate-in fade-in">
                  <div className="flex justify-between items-center"><h1 className="text-xl font-bold flex items-center gap-3 italic text-slate-800"><History className="text-blue-600" /> Auditoria Geral</h1><button onClick={() => { const h="Data,Usuário,Ação,Detalhes\n"; const r=auditLogs.map(l=>`${new Date(l.log_date).toLocaleString()},${l.username},${l.action},${l.details}`).join("\n"); const b=new Blob([h+r],{type:'text/csv;charset=utf-8;'}); const l=document.createElement("a"); l.href=URL.createObjectURL(b); l.setAttribute("download",`auditoria_inepad.csv`); l.click(); }} className="bg-slate-900 text-white px-4 py-2 rounded-xl text-[9px] font-black uppercase flex items-center gap-2 hover:bg-black transition-all"><FileText size={14}/> Exportar CSV</button></div>
                  <div className="bg-white rounded-[40px] border shadow-sm overflow-hidden">
                    <table className="w-full text-left text-xs"><thead className="bg-slate-50 text-slate-400 uppercase font-bold border-b"><tr><th className="p-6">Data</th><th className="p-6">Usuário</th><th className="p-6">Ação</th><th className="p-6">Detalhes</th></tr></thead>
                    <tbody>{auditLogs.map((log:any)=>(<tr key={log.id} className="border-t"><td className="p-6 font-bold text-slate-400 italic">{new Date(log.log_date).toLocaleString()}</td><td className="p-6 text-blue-600 font-black uppercase">{log.username}</td><td className="p-6"><span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full font-black uppercase text-[9px]">{log.action}</span></td><td className="p-6 text-slate-600 font-bold italic">{log.details}</td></tr>))}</tbody></table>
                  </div>
                </div>
              )}
            </>
