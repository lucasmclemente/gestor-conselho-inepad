...
const App = () => {
  ...
  const [meetings, setMeetings] = useState<any[]>([
    { 
      id: 1, title: 'Reunião de Conselho - Exemplo', status: 'Concluída', date: '2025-12-10', time: '10:00', type: 'Presencial',
      participants: [{name: 'Ricardo Oliveira', email: 'r.oliveira@inepad.com.br'}],
      pautas: [{title: 'Aprovação de Orçamento', dur: '20', resp: 'Ricardo Oliveira'}],
  ...
  const [auditLogs, setAuditLogs] = useState<any[]>([
    { id: 1, date: '19/02/2026 14:15', user: 'Sistema', action: 'Correção', details: 'Abas Deliberações e Atas restauradas.' }
  ]);
  ...
  const stats = useMemo(() => {
    ...
    const pendingList = allA.filter(a => a.status !== 'Concluída');
    return {
      ...
      atrasadas: allA.filter(a => a.status !== 'Concluída' && a.date && new Date(a.date) < today).length,
      allActions: allA,
      pieData: [
        { name: 'Em Andamento', value: allA.filter(a=>a.status==='Em andamento').length, color: '#3b82f6' },
        { name: 'Pendente', value: allA.filter(a=>a.status==='Pendente').length, color: '#f59e0b' },
        { name: 'Em Atraso', value: allA.filter(a => a.status !== 'Concluída' && a.date && new Date(a.date) < today).length, color: '#ef4444' }
      ],
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-white rounded-[40px] shadow-2xl p-10">
          ...
          <form className="space-y-4" onSubmit={(e)=>{e.preventDefault(); const u=users.find(x=>x.email===authForm.email); if(u){setCurrentUser(u); setActiveMenu('dashboard');} else alert('Erro!');}}>
            ...
            <button className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black uppercase">Entrar</button>
          </form>
        </div>
      </div>
    );
  ...
  if (activeMenu === 'dashboard') {
    mainContent = (
      <div className="space-y-8 animate-in fade-in">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[320px]">
          <div className="bg-white p-8 rounded-[40px] border shadow-sm flex flex-col h-full">
            <h3 className="text-[10px] font-black uppercase mb-6 text-slate-500">Status das Ações</h3>
            <div className="flex-1"><ResponsiveContainer><PieChart><Pie data={stats.pieData} innerRadius={50} outerRadius={70} dataKey="value" paddingAngle={5}>{stats.pieData.map((e,i)=>(<Cell key={i} fill={e.color}/>))}</Pie><Tooltip/><Legend/></PieChart></ResponsiveContainer></div>
          </div>
          <div className="bg-white p-8 rounded-[40px] border shadow-sm flex flex-col h-full">
            <h3 className="text-[10px] font-black uppercase mb-6 text-slate-500">Decisões por Reunião</h3>
            <div className="flex-1"><ResponsiveContainer><BarChart data={stats.barData}><CartesianGrid vertical={false} stroke="#f1f5f9"/><XAxis dataKey="name" tick={{fontSize:9}}/><YAxis hide/><Tooltip/><Bar dataKey="Itens em Pauta" fill="#4f46e5" radius={[4,4,0,0]} barSize={20}/><Bar dataKey="Ações no Plano" fill="#10b981" radius={[4,4,0,0]} barSize={20}/></BarChart></ResponsiveContainer></div>
          </div>
        </div>
    ...
    mainContent = (
      <div className="space-y-6 animate-in fade-in"><button onClick={() => setActiveMenu('dashboard')} className="text-slate-400 flex items-center gap-1 text-[10px] font-black uppercase italic"><ChevronRight className="rotate-180" size={16}/> Voltar</button><div className="bg-white p-8 rounded-[40px] border shadow-lg overflow-hidden"><h2 className="text-xl font-black italic mb-6">Plano de Ações Integral</h2><table className="w-full text-left text-xs"><thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400 border-b"><tr><th className="p-6">Ação</th><th className="p-6">Origem</th><th className="p-6">Status</th><th className="p-6">Dono</th></tr></thead><tbody>{stats.allActions.map((a:any, i:number)=>(<tr key={i} className="border-t hover:bg-slate-50"><td className="p-6 text-slate-800 font-bold">{a.title}</td><td className="p-6 text-slate-400 italic">{a.mTitle}</td><td className="p-6 font-bold">{a.status}</td><td className="p-6 text-blue-600 uppercase font-black">{a.resp}</td></tr>))}</tbody></table></div></div>
    );
  } else if (activeMenu === 'reunioes') {
    mainContent = view === 'list' ? (
      <div className="space-y-6 animate-in fade-in">
        <div className="flex justify-between items-center"><h1 className="text-2xl font-black italic tracking-tighter">Reuniões</h1><button onClick={()=>{setCurrentMeeting({id:0, title:'', status:'Agendada', participants:[], pautas:[], materiais:[], deliberacoes:[], acoes:[], atas:[]}); setView('details'); setTab('info');}} className="bg-blue-600 text-white px-6 py-2.5 rounded-2xl font-black text-xs uppercase shadow-xl hover:scale-105 transition-all">+ Nova Reunião</button></div>
      ...
      <div className="animate-in fade-in pb-20">
        ...
        {tab === 'info' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in">
            <div className="bg-white p-6 rounded-[32px] border shadow-sm"><h3 className="text-[10px] font-black uppercase mb-4 italic text-slate-400">Participantes Convidados</h3><div className="space-y-2 mb-4">{(currentMeeting.participants || []).map((p:any, i:any) => (<div key={i} className="flex justify-between p-3 bg-slate-50 rounded-2xl text-xs font-bold italic"><span>{p.name}</span><button onClick={()=>setCurrentMeeting({...currentMeeting, participants: currentMeeting.participants.filter((_:any, idx:any)=>idx!==i)})}><X size={14}/></button></div>))}</div><div className="space-y-2 p-3 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200"><input placeholder="Nome" className="w-full p-2 text-xs border rounded-xl outline-none" value={tmpPart.name} onChange={e=>setTmpPart({...tmpPart, name:e.target.value})} /><button onClick={()=>{if(tmpPart.name){setCurrentMeeting({...currentMeeting, participants:[...(currentMeeting.participants || []), tmpPart]}); setTmpPart({name:'', email:''});}}} className="w-full py-2 bg-blue-600 text-white rounded-xl text-[9px] font-black uppercase shadow-sm">Adicionar</button></div><button onClick={()=>alert("Convocação oficial enviada!")} className="w-full mt-4 py-3 border-2 border-blue-600 text-blue-600 rounded-2xl text-[9px] font-black uppercase flex items-center justify-center gap-2"><Send size={14}/> Convocação Oficial</button></div>
        ...
        {tab === 'atas' && (
          <div className="bg-white p-8 rounded-[40px] border shadow-sm animate-in fade-in space-y-8">
            <div className="flex justify-between items-center border-b pb-4"><h3 className="text-[10px] font-black uppercase flex items-center gap-2 italic text-green-700"><FileCheck size={18}/> Documentação Oficial (Atas)</h3><button onClick={()=>ataRef.current?.click()} className="bg-green-600 text-white px-6 py-3 rounded-2xl text-[9px] font-black uppercase shadow-xl transition-all hover:scale-105"><Upload size={14} className="inline mr-2"/>Subir Ata Final</button><input type="file" ref={ataRef} className="hidden" onChange={(e)=>{const f=e.target.files?.[0]; if(f){setCurrentMeeting({...currentMeeting, atas: [...(currentMeeting.atas || []), {id: Date.now(), name: f.name}]}); addLog('Ata', `Documento: ${f.name}`);}}} /></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{(currentMeeting.atas || []).map((ata:any, i:any) => (<div key={i} className="p-5 bg-green-50 border border-green-100 rounded-[32px] flex items-center gap-4 group shadow-sm"><FileCheck size={28} className="text-green-600"/><div className="flex-1 min-w-0 font-bold italic"><p className="text-sm text-slate-700 truncate tracking-tight">{ata.name}</p></div><button onClick={()=>setCurrentMeeting({...currentMeeting, atas: currentMeeting.atas.filter((_:any, idx:any) => idx !== i)})} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={18}/></button></div>))}</div>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans overflow-hidden animate-in fade-in">
