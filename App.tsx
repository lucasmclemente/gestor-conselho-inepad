...
const App = () => {
  ...
  const stats = useMemo(() => {
    ...
    const pendingList = allA.filter(a => a.status !== 'Concluído'); // Filtro de Concluídos
  ...
  // --- RENDERIZADORES DE TABELA (BLINDAGEM DE SINTAXE) ---
  const ActionTableRows = (limit?: number) => {
    const data = limit ? stats.pendingList.slice(0, limit) : stats.pendingList;
    if (data.length === 0) return <tr><td colSpan={4} className="p-10 text-center text-slate-300 italic uppercase text-[10px]">Nenhuma pendência ativa.</td></tr>;
  ...
  // --- LÓGICA DE TELAS (RESOLVE O ERRO DO VERCEL) ---
  let mainContent;
  ...
  if (activeMenu === 'dashboard') {
      ...
      <div className="space-y-8 animate-in fade-in">
        ...
        <div className="bg-white p-8 rounded-[40px] border shadow-sm overflow-hidden">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-[10px] font-black uppercase italic text-slate-500 flex items-center gap-2"><Target size={16}/> Plano de Ações (Top 5 Pendências)</h3>
          ...
          <table className="w-full text-left font-bold italic">
            <thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400 border-b italic"><tr><th className="p-4">Ação</th><th className="p-4">Reunião</th><th className="p-4 text-center">Status</th><th className="p-4">Dono</th></tr></thead>
            <tbody>{ActionTableRows(5)}</tbody>
          </table>
        </div>
      </div>
    );
    ...
    mainContent = (
      <div className="space-y-6 animate-in fade-in">
        ...
        <div className="bg-white p-8 rounded-[40px] border shadow-lg overflow-hidden">
          ...
          <tbody>{ActionTableRows()}</tbody></table>
        </div>
      </div>
    );
  } else if (view === 'list') {
    ...
    mainContent = (
      <div className="animate-in fade-in pb-20">
        ...
        {tab === 'pauta' && (
          <div className="bg-white p-8 rounded-[40px] border shadow-sm animate-in fade-in space-y-6"><div className="flex justify-between items-center font-black uppercase text-[10px] italic text-slate-500"><h3>Ordem do Dia</h3><span className="text-blue-600 bg-blue-50 px-3 py-1 rounded-full">Total: {currentMeeting.pautas.reduce((acc, p) => acc + Number(p.duration || 0), 0)} min</span></div><div className="space-y-3">{currentMeeting.pautas.map((p, i) => (<div key={i} className="flex gap-4 p-5 border rounded-3xl bg-slate-50/50 text-xs items-center shadow-sm font-bold italic"><GripVertical className="text-slate-200"/><div className="flex-1">{p.title}</div><div className="font-black text-blue-600">{p.duration} MIN</div><button onClick={()=>setCurrentMeeting({...currentMeeting, pautas: currentMeeting.pautas.filter((_, idx)=>idx!==i)})}><Trash2 size={16}/></button></div>))}</div><button onClick={()=>setCurrentMeeting({...currentMeeting, pautas:[...currentMeeting.pautas, {title: 'Novo Item', duration: '15'}]})} className="w-full py-3 bg-blue-600 text-white rounded-2xl font-black text-[10px] uppercase shadow-lg">+ Adicionar Pauta</button></div>
        )}
        {tab === 'materiais' && (
  ...
  // --- RENDERIZAÇÃO FINAL (MINIMALISTA PARA EVITAR ERROS) ---
  return (
