...
const App = () => {
  ...
  const [auditLogs, setAuditLogs] = useState<any[]>([
    { id: 1, date: '19/02/2026 11:55', user: 'Sistema', action: 'Estabilidade', details: 'Interface integral restabelecida e bugs de renderização corrigidos.' }
  ]);
  ...
  const [meetings, setMeetings] = useState([
    { 
      ...
      acoes: [{id: 201, title: 'Definir branding', date: '2026-01-30', status: 'Concluído', resp: 'Admin'}], 
      atas: []
    }
  ]);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      del: allD.filter(d => d.votes?.some((v:any) => v.status === 'Aprovado')).length,
      pendingList,
      pieData: [
        ...
        <div className="w-full max-w-md bg-white rounded-[40px] shadow-2xl overflow-hidden">
          ...
          <div className="p-10">
            <form onSubmit={(e) => {
              ...
              <div className="relative"><Mail className="absolute left-4 top-3.5 text-slate-400" size={18} /><input type="email" placeholder="E-mail" className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border rounded-2xl text-sm font-bold outline-none" value={authForm.email} onChange={e=>setAuthForm({...authForm, email: e.target.value})} /></div>
              <div className="relative"><Key className="absolute left-4 top-3.5 text-slate-400" size={18} /><input type="password" placeholder="Senha" className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border rounded-2xl text-sm font-bold outline-none" value={authForm.password} onChange={e=>setAuthForm({...authForm, password: e.target.value})} /></div>
