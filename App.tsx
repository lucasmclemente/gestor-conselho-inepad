...
const App = () => {
  ...
  const handleSave = () => {
    if (!currentMeeting.title) return alert("Defina o título!");
    const finalEntry = currentMeeting.id === 0 ? { ...currentMeeting, id: Date.now() } : currentMeeting;
    setMeetings(currentMeeting.id === 0 ? [finalEntry, ...meetings] : meetings.map(m => m.id === finalEntry.id ? finalEntry : m));
  ...
  const handleUpload = (e: any, target: 'materiais' | 'atas') => {
    ...
    if (file) {
      const entry = { id: Date.now(), name: file.name, uploadedBy: 'Ricardo Oliveira', date: '18/02/2026' };
      setCurrentMeeting(prev => ({ ...prev, [target]: [...prev[target], entry] }));
    }
  };

  // --- RENDERIZADORES DE TABELAS ---
  const renderDashAcoes = stats.pending.slice(0, 5).map((a, i) => (
  ...
  // --- COMPOSIÇÃO DE CONTEÚDO (LINEAR PARA EVITAR ERRO DE BUILD) ---
  let mainContent;
    ...
    mainContent = (
      <div className="space-y-8 animate-in fade-in">
        ...
        <div className="bg-white p-8 rounded-[40px] border shadow-sm overflow-hidden">
          <div className="flex justify-between items-center mb-6"><h3 className="text-[10px] font-black uppercase italic text-slate-500 flex items-center gap-2"><Target size={16}/> Plano de Ações (Top 5 Pendências)</h3><button onClick={() => setActiveMenu('plano-geral')} className="text-blue-600 text-[10px] font-black uppercase flex items-center gap-1 hover:underline">Ver Todas <ExternalLink size={12}/></button></div>
          ...
          <tbody>{renderDashAcoes}</tbody></table>
        </div>
      </div>
    );
