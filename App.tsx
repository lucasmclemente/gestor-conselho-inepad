...
import { 
  ...
  Mail, UserCog, ShieldCheck, ShieldAlert, Settings, Camera, UserCircle, KeyRound, ListChecks, History
} from 'lucide-react';
...
const App = () => {
  ...
  const [auditLogs, setAuditLogs] = useState<any[]>([
    { id: 1, date: '18/02/2026 14:00', user: 'Sistema', action: 'Inicialização', details: 'Base de dados carregada.' }
  ]);
  ...
  const handleLogin = (e: any) => {
    ...
    if (user) { 
      ...
      // addLog não funciona aqui porque currentUser ainda é null na execução, mas o log de auditoria registrará ações pós-login
  ...
  const handleCreateUser = () => {
    if (!newUserForm.name || !newUserForm.email) return alert("Dados incompletos.");
    setUsers([...users, { ...newUserForm, id: Date.now() }]);
    addLog('Gestão Usuário', `Novo usuário: ${newUserForm.name} (${newUserForm.role})`);
      ...
      <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-6 font-sans">
        <div className="w-full max-w-md bg-white rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in duration-300">
          ...
          <div className="p-10">
            ...
            <button onClick={() => setAuthState(authState === 'login' ? 'register' : 'login')} className="w-full mt-6 text-slate-400 text-[10px] font-black uppercase text-center">{authState === 'login' ? 'Solicitar acesso' : 'Voltar ao Login'}</button>
          </div>
        </div>
      </div>
    );
  ...
  if (activeMenu === 'dashboard') {
    mainContent = (
      <div className="space-y-8 animate-in fade-in">
        <div className="flex justify-between items-end"><h1 className="text-2xl font-black italic text-slate-800 tracking-tighter">Dashboard Estratégico</h1>{canCreate && <button onClick={()=>{setCurrentMeeting({...blankMeeting, id:0}); setView('details'); setTab('info'); setActiveMenu('reunioes');}} className="bg-blue-600 text-white px-5 py-2.5 rounded-2xl font-black text-[10px] uppercase shadow-xl hover:scale-105 transition-all tracking-widest">+ Nova Reunião</button>}</div>
        ...
        <div className="bg-white rounded-[40px] border shadow-sm overflow-hidden">
          <table className="w-full text-left font-bold italic">
            <thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400 border-b">
              <tr><th className="p-6">Data/Hora</th><th className="p-6">Usuário</th><th className="p-6">Ação</th><th className="p-6">Detalhes</th></tr>
            </thead>
            <tbody>
              {auditLogs.map((log) => (
                <tr key={log.id} className="border-t hover:bg-slate-50/50 transition-colors">
                  <td className="p-6 text-[10px] text-slate-400 font-black">{log.date}</td>
                  <td className="p-6 text-xs text-blue-600 font-black uppercase">{log.user}</td>
                  <td className="p-6"><span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase ${log.action === 'Exclusão' ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'}`}>{log.action}</span></td>
      ...
      <div className="space-y-8 animate-in fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-white p-6 rounded-[32px] border shadow-sm space-y-4 h-fit sticky top-0">
             ...
             <button onClick={handleCreateUser} className="w-full py-3 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase shadow-lg">Criar Usuário</button>
          </div>
          <div className="md:col-span-3 bg-white rounded-[40px] border shadow-sm overflow-hidden"><table className="w-full text-left font-bold italic"><thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400 border-b"><tr><th className="p-6">Usuário</th><th className="p-6">Perfil</th><th className="p-6 text-center">Ações</th></tr></thead><tbody>{users.map((u, i) => (
            <tr key={i} className="border-t"><td className="p-6 text-xs">{u.name}<p className="text-[9px] text-slate-400 font-black">{u.email}</p></td><td className="p-6"><span className="bg-slate-50 p-2 rounded-xl text-[9px] font-black uppercase">{u.role}</span></td><td className="p-6 text-center"><button onClick={()=>{if(u.id!==1){ setUsers(users.filter(x=>x.id!==u.id)); addLog('Exclusão', `Usuário removido: ${u.name}`); }}} className="text-red-300 hover:text-red-500 transition-colors"><Trash2 size={18}/></button></td></tr>
          ))}</tbody></table></div>
        </div>
      </div>
    );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-1 bg-white p-8 rounded-[40px] border shadow-sm flex flex-col items-center text-center">
            <div className="relative mb-6 cursor-pointer" onClick={()=>profilePicRef.current?.click()}>
              ...
              <input type="file" ref={profilePicRef} className="hidden" onChange={(e)=>{const f=e.target.files?.[0]; if(f){const r=new FileReader(); r.onloadend=()=>{const b=r.result; setUsers(users.map(u=>u.id===currentUser.id?{...u, avatar:b}:u)); setCurrentUser({...currentUser, avatar:b}); addLog('Perfil', 'Foto de perfil atualizada');}; r.readAsDataURL(f);}}} />
            </div>
          ...
          <div className="md:col-span-2 bg-white p-8 rounded-[40px] border shadow-sm space-y-6">
            ...
            <button onClick={()=>{setUsers(users.map(u=>u.id===currentUser.id?{...u, name:profileForm.name, email:profileForm.email}:u)); setCurrentUser({...currentUser, name:profileForm.name, email:profileForm.email}); addLog('Perfil', 'Dados de contato atualizados'); alert("Perfil salvo!");}} className="bg-slate-900 text-white px-8 py-3.5 rounded-2xl font-black text-[10px] uppercase flex items-center gap-2"><Save size={16}/> Atualizar Dados</button>
          </div>
        </div>
      </div>
    );
