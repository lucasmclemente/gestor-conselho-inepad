...
const App = () => {
  ...
  const dashboardData = useMemo(() => {
    ...
    const allActions = meetings.flatMap(m => (m.acoes || []).map(a => ({ ...a, mTitle: m.title })));
    const allDelibs = meetings.flatMap(m => m.deliberacoes || []);
    
    const completed = allActions.filter(a => a.status === 'Concluído').length;
    const approved = allDelibs.filter(d => d.votes?.some((v:any) => v.status.includes('Aprovado'))).length;
    ...
    return { 
      act: `${completed}/${allActions.length}`, 
      del: `${approved}/${allDelibs.length}`, 
      atas: totalAtas, 
      delay: delayed,
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-[#f8fafc]">
          ...
          {activeMenu === 'dashboard' && (
            <div className="space-y-8 animate-in fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                  {l:'Ações Concluídas', v:dashboardData.act, i:<CheckCircle2/>, c:'blue'},
                  {l:'Deliberações Aprovadas', v:dashboardData.del, i:<FileText/>, c:'indigo'},
                  {l:'ATAs Registradas', v:dashboardData.atas, i:<FileCheck/>, c:'green'},
                  {l:'Ações Atrasadas', v:dashboardData.delay, i:<AlertCircle/>, c:'red'}
                ].map((s, i) => (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-white p-8 rounded-[40px] border shadow-sm h-96 flex flex-col">
                  ...
                  <div className="flex-1"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={[{n:'OK', v:10}, {n:'Atrasadas', v:dashboardData.delay}]} innerRadius={60} outerRadius={80} dataKey="v" paddingAngle={8}><Cell fill="#10b981"/><Cell fill="#ef4444"/></Pie><Tooltip/></PieChart></ResponsiveContainer></div>
              ...
              <div className="bg-white p-8 rounded-[40px] border shadow-sm overflow-hidden">
                ...
                <table className="w-full text-left"><thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400 border-b italic"><tr><th className="p-4">Ação</th><th className="p-4">Reunião</th><th className="p-4">Status da Ação</th><th className="p-4">Responsável</th></tr></thead><tbody className="text-xs font-bold italic">{dashboardData.allActions.map((a, i) => (<tr key={i} className="border-t hover:bg-slate-50/50"><td className="p-4 text-slate-800 underline decoration-blue-100">{a.title}</td><td className="p-4 text-slate-400 text-[10px] uppercase font-black">{a.mTitle}</td><td className="p-4"><span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase ${a.status==='Concluído'?'bg-green-100 text-green-700':'bg-blue-100 text-blue-700'}`}>{a.status}</span></td><td className="p-4 text-slate-500">{a.resp}</td></tr>))}</tbody></table>
              </div>
            </div>
          )}
          ...
          {activeMenu === 'reunioes' && (view === 'list' ? (
            ...
            <div className="animate-in fade-in pb-20">
              ...
              {/* EDITOR TABS (SINTAXE SIMPLIFICADA) */}
              {tab === 'info' && (
                <div className="space-y-6 animate-in fade-in">
