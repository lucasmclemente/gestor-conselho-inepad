...
const App = () => {
  ...
  // --- LÓGICA DE DADOS (HOJE: 18/02/2026) ---
  const stats = useMemo(() => {
  ...
  const [newDelib, setNewDelib] = useState({ title: '' });
  ...
  const AcoesTable = ({ limit }: { limit?: number }) => {
    ...
    return (
      <table className="w-full text-left font-bold italic">
        <thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400 border-b italic">
          <tr><th className="p-4">Ação</th><th className="p-4">Reunião</th><th className="p-4">Status</th><th className="p-4">Dono</th></tr>
        </thead>
  ...
  // --- TELAS (MODULARIZADAS PARA EVITAR ERRO DE SINTAXE) ---
  const DashboardView = () => (
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-[#f8fafc]">
          ...
          {activeMenu === 'plano-geral' && (
            <div className="space-y-6 animate-in fade-in">
              ...
              <div className="bg-white p-8 rounded-[40px] border shadow-lg overflow-hidden">
                <h2 className="text-xl font-black italic text-slate-800 mb-6 flex items-center gap-2"><Target className="text-blue-600"/> Plano de Ações Completo</h2>
                <AcoesTable />
              </div>
            </div>
          )}
          ...
          {activeMenu === 'reunioes' && (view === 'list' ? (
            ...
            <div className="animate-in fade-in pb-20">
              ...
              {tab === 'atas' && (
                <div className="bg-white p-8 rounded-[40px] border shadow-sm animate-in fade-in space-y-8"><div className="flex justify-between items-center border-b pb-4"><div><h3 className="text-[10px] font-black uppercase flex items-center gap-2 italic text-green-700 tracking-widest"><FileCheck size={18}/> Atas Finais</h3><p className="text-[8px] text-slate-400 font-black uppercase mt-1 italic tracking-widest">Documentação oficial assinada</p></div><button onClick={()=>ataRef.current?.click()} className="bg-green-600 text-white px-6 py-3 rounded-2xl text-[9px] font-black uppercase shadow-xl transition-all hover:scale-105"><Upload size={14} className="inline mr-2"/>Subir</button><input type="file" ref={ataRef} className="hidden" onChange={(e)=>handleUpload(e, 'atas')} /></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{currentMeeting.atas.map((ata, i) => (<div key={i} className="p-5 bg-green-50/50 border border-green-100 rounded-[32px] flex items-center gap-4 group transition-all hover:border-green-300 shadow-sm"><FileCheck size={28} className="text-green-600 shadow-sm"/><div className="flex-1 min-w-0 font-bold italic"><p className="text-sm text-slate-700 truncate tracking-tight">{ata.name}</p></div><button onClick={()=>setCurrentMeeting({...currentMeeting, atas: currentMeeting.atas.filter((_, idx) => idx !== i)})} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={18}/></button></div>))}</div></div>
              )}
            </div>
          )}
        </div>
