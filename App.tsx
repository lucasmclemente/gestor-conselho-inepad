...
const App = () => {
  ...
  const [auditLogs, setAuditLogs] = useState<any[]>([
    { id: 1, date: '19/02/2026 11:45', user: 'Sistema', action: 'Restauração', details: 'Motor de renderização completo reativado.' }
  ]);
  ...
  const [meetings, setMeetings] = useState([
    { 
      id: 1, title: 'Conselho de Administração - Q1', status: 'AGENDADA', date: '2026-03-10', time: '10:00', type: 'Presencial', address: 'Sede INEPAD',
      participants: [{name: 'Membro Exemplo', email: 'contato@inepad.com.br'}],
      pautas: [{title: 'Aprovação de Orçamento', dur: '30', resp: 'Administrador'}],
  ...
  const stats = useMemo(() => {
    ...
    const approvedDelibs = allD.filter(d => d.votes?.some((v:any) => v.status === 'Aprovado')).length;
  ...
  const handleSaveMeeting = () => {
    ...
    addLog(isNew ? 'Criação' : 'Edição', `Reunião: ${currentMeeting.title}`);
  };
  ...
  const handleUpload = (e: any, target: 'materiais' | 'atas') => {
    ...
    if (f) {
      ...
      addLog('Upload', `Documento em ${target}: ${f.name}`);
    }
  };
        ...
        <div className="w-full max-w-md bg-white rounded-[40px] shadow-2xl overflow-hidden">
          ...
          <div className="p-10">
            <form onSubmit={handleLogin} className="space-y-4">
              <div className="relative"><Mail className="absolute left-4 top-3.5 text-slate-400" size={18} /><input type="email" placeholder="E-mail" className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border rounded-2xl text-sm font-bold outline-none focus:border-blue-500" value={authForm.email} onChange={e=>setAuthForm({...authForm, email: e.target.value})} /></div>
              <div className="relative"><Key className="absolute left-4 top-3.5 text-slate-400" size={18} /><input type="password" placeholder="Senha" className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border rounded-2xl text-sm font-bold outline-none focus:border-blue-500" value={authForm.password} onChange={e=>setAuthForm({...authForm, password: e.target.value})} /></div>
              <button type="submit" className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black text-xs uppercase shadow-xl hover:bg-blue-700 transition-all">Acessar Painel</button>
            </form>
          </div>
        </div>
      </div>
  ...
  // --- REUNIÕES ---
    ...
    mainContent = view === 'list' ? (
      ...
      <div className="animate-in fade-in pb-20">
        ...
        <input placeholder="Título da Reunião..." className="text-3xl font-black text-slate-800 bg-transparent outline-none w-full italic mb-8 border-b-2 border-slate-100 focus:border-blue-300 pb-2" value={currentMeeting.title} onChange={e=>setCurrentMeeting({...currentMeeting, title: e.target.value})} />
