...
import { 
  ...
  Mail, UserCog, Settings, Camera, UserCircle, History, ShieldCheck
} from 'lucide-react';
...
const App = () => {
  ...
  const [meetings, setMeetings] = useState<any[]>([
    { 
      id: 1, title: 'Reunião Ordinária do Conselho - Q4', status: 'Concluída', date: '2025-12-10', time: '10:00', type: 'Presencial',
      participants: [{name: 'Beatriz Costa', email: 'b.costa@inepad.com.br'}],
      pautas: [{title: 'Aprovação de Dividendos', dur: '20', resp: 'Beatriz Costa'}],
      materiais: [], deliberacoes: [], 
      acoes: [{id: 201, title: 'Comunicar acionistas', date: '2026-03-20', status: 'Concluída', resp: 'Beatriz Costa'}], 
      atas: [{name: 'Ata_Final.pdf'}]
    }
  ]);
  ...
  const [auditLogs, setAuditLogs] = useState<any[]>([
    { id: 1, date: '19/02/2026 14:00', user: 'Sistema', action: 'Restauração', details: 'Interface completa sincronizada.' }
  ]);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      allActions: allA,
      pieData: [
        { name: 'Em Andamento', value: andamentoCount, color: '#3b82f6' },
        { name: 'Pendente', value: pendenteCount, color: '#f59e0b' },
        { name: 'Em Atraso', value: atrasadaCount, color: '#ef4444' }
      ],
      barData: meetings.map(m => ({
        name: m.title?.substring(0, 8),
        'Itens em Pauta': m.pautas?.length || 0,
        'Ações no Plano': m.acoes?.length || 0
      }))
    };
  }, [meetings]);
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-white rounded-[40px] shadow-2xl overflow-hidden">
          <div className="p-10 bg-slate-800 text-white text-center border-b-8 border-blue-600">
          ...
          <div className="p-10">
            <form onSubmit={(e) => {
              e.preventDefault();
              const u = users.find(x => x.email === authForm.email);
              if (u) { setCurrentUser(u); setActiveMenu('dashboard'); }
              else alert('Acesso Negado');
            }} className="space-y-4">
              <input type="email" placeholder="E-mail" className="w-full p-4 bg-slate-50 border rounded-2xl outline-none" onChange={e=>setAuthForm({...authForm, email:e.target.value})} />
              <input type="password" placeholder="Senha" className="w-full p-4 bg-slate-50 border rounded-2xl outline-none" />
              <button className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black uppercase text-xs">Entrar no Painel</button>
            </form>
          </div>
        </div>
      </div>
  ...
  if (activeMenu === 'dashboard') {
      ...
      <div className="space-y-8 animate-in fade-in">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[360px]">
          <div className="bg-white p-8 rounded-[40px] border shadow-sm flex flex-col">
            ...
            <div className="flex-1 min-h-0"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={stats.pieData} innerRadius={60} outerRadius={85} dataKey="value" paddingAngle={5}>{stats.pieData.map((e,i)=>(<Cell key={i} fill={e.color}/>))}</Pie><Tooltip/><Legend/></PieChart></ResponsiveContainer></div>
          </div>
          <div className="bg-white p-8 rounded-[40px] border shadow-sm flex flex-col">
            <h3 className="text-[10px] font-black uppercase mb-6 italic text-slate-500">Decisões por Reunião</h3>
            <div className="flex-1 min-h-0"><ResponsiveContainer width="100%" height="100%"><BarChart data={stats.barData}><CartesianGrid vertical={false} stroke="#f1f5f9"/><XAxis dataKey="name" tick={{fontSize:9}}/><YAxis hide/><Tooltip/><Legend/><Bar dataKey="Itens em Pauta" fill="#4f46e5" radius={[4,4,0,0]} barSize={20}/><Bar dataKey="Ações no Plano" fill="#10b981" radius={[4,4,0,0]} barSize={20}/></BarChart></ResponsiveContainer></div>
          </div>
        </div>
    ...
    mainContent = (
      <div className="space-y-6 animate-in fade-in"><button onClick={() => setActiveMenu('dashboard')} className="text-slate-400 flex items-center gap-1 text-[10px] font-black uppercase italic"><ChevronRight className="rotate-180" size={16}/> Voltar</button><div className="bg-white p-8 rounded-[40px] border shadow-lg overflow-hidden"><h2 className="text-xl font-black italic mb-6">Plano de Ações Integral</h2><table className="w-full text-left font-bold italic text-xs"><thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400 border-b"><tr><th className="p-6">Ação</th><th className="p-6">Origem</th><th className="p-6">Status</th><th className="p-6">Responsável</th></tr></thead><tbody>{stats.allActions.map((a:any, i:number)=>(<tr key={i} className="border-t hover:bg-slate-50"><td className="p-6 text-slate-800 font-bold">{a.title}</td><td className="p-6 text-slate-400 uppercase font-black">{a.mTitle}</td><td className="p-6"><span className="bg-blue-100 px-4 py-1.5 rounded-full text-[8px] font-black uppercase">{a.status}</span></td><td className="p-6 font-black text-slate-600 uppercase">{a.resp}</td></tr>))}</tbody></table></div></div>
    );
  }
      ...
      <div className="animate-in fade-in pb-20">
        ...
        {tab === 'info' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in">
            <div className="bg-white p-6 rounded-[32px] border shadow-sm"><h3 className="text-[10px] font-black uppercase mb-4 italic text-slate-400">Participantes Convidados</h3><div className="space-y-2 mb-4">{(currentMeeting.participants || []).map((p:any, i:any) => (<div key={i} className="flex justify-between p-3 bg-slate-50 rounded-2xl text-xs font-bold italic"><span>{p.name}</span><button onClick={()=>setCurrentMeeting({...currentMeeting, participants: currentMeeting.participants.filter((_:any, idx:any)=>idx!==i)})}><X size={14}/></button></div>))}</div><div className="space-y-2 p-3 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200"><input placeholder="Nome" className="w-full p-2 text-xs border rounded-xl outline-none" value={tmpPart.name} onChange={e=>setTmpPart({...tmpPart, name:e.target.value})} /><button onClick={()=>{if(tmpPart.name){setCurrentMeeting({...currentMeeting, participants:[...(currentMeeting.participants || []), tmpPart]}); setTmpPart({name:'', email:''});}}} className="w-full py-2 bg-blue-600 text-white rounded-xl text-[9px] font-black uppercase">Adicionar</button></div><button onClick={()=>alert("E-mails enviados!")} className="w-full mt-4 py-3 border-2 border-blue-600 text-blue-600 rounded-2xl text-[9px] font-black uppercase flex items-center justify-center gap-2"><Send size={14}/> Convocação Oficial</button></div>
        ...
        {tab === 'acoes' && (
          <div className="bg-white p-8 rounded-[40px] border shadow-sm animate-in fade-in space-y-6"><h3 className="text-[10px] font-black uppercase italic text-slate-500">Plano de Ação da Reunião</h3><div className="grid grid-cols-3 gap-4 p-6 bg-slate-50 rounded-[32px]"><input placeholder="Descrição" className="col-span-3 p-3 text-xs border rounded-xl outline-none" value={tmpAcao.title} onChange={e=>setTmpAcao({...tmpAcao, title: e.target.value})} />
  ...
  // --- MEMBROS ---
    ...
    mainContent = (
      <div className="space-y-8 animate-in fade-in"><h1 className="text-2xl font-black italic text-slate-800 tracking-tighter">Membros da INEPAD</h1><div className="grid grid-cols-1 md:grid-cols-4 gap-6"><div className="bg-white p-6 rounded-[32px] border shadow-sm space-y-4 h-fit sticky top-0"><h3 className="text-[10px] font-black uppercase italic text-slate-400 tracking-widest">Novo Perfil</h3><input placeholder="Nome" className="w-full p-3 bg-slate-50 border rounded-xl text-xs font-bold outline-none" value={newUserForm.name} onChange={e=>setNewUserForm({...newUserForm, name: e.target.value})} /><input placeholder="Email" className="w-full p-3 bg-slate-50 border rounded-xl text-xs font-bold outline-none" value={newUserForm.email} onChange={e=>setNewUserForm({...newUserForm, email: e.target.value})} /><input type="password" placeholder="Senha" className="w-full p-3 bg-slate-50 border rounded-xl text-xs font-bold outline-none" value={newUserForm.password} onChange={e=>setNewUserForm({...newUserForm, password: e.target.value})} /><select className="w-full p-3 border rounded-xl text-[10px] font-black uppercase italic bg-white" value={newUserForm.role} onChange={e=>setNewUserForm({...newUserForm, role: e.target.value})}><option value="Conselheiro">Conselheiro</option><option value="Secretário">Secretário</option><option value="Administrador">Administrador</option></select><button onClick={()=>{setUsers([...users, {...newUserForm, id: Date.now()}]); addLog('Segurança', `Membro: ${newUserForm.name}`); alert("Cadastrado com Sucesso!");}} className="w-full py-3 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase shadow-lg">Confirmar Cadastro</button></div><div className="md:col-span-3 bg-white rounded-[32px] border shadow-sm overflow-hidden"><table className="w-full text-left font-bold italic text-xs"><thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400 border-b"><tr><th className="p-6">Nome / E-mail</th><th className="p-6 text-center">Perfil</th><th className="p-6 text-center">Ações</th></tr></thead><tbody>{users.map((u, i) => (<tr key={i} className="border-t hover:bg-slate-50"><td className="p-6 text-xs">{u.name}<p className="text-[9px] text-slate-400 font-black tracking-widest">{u.email}</p></td><td className="p-6 text-center"><span className="bg-slate-100 px-4 py-1.5 rounded-xl text-[9px] font-black uppercase">{u.role}</span></td><td className="p-6 text-center"><button onClick={()=>{if(u.id!==1) setUsers(users.filter(x=>x.id!==u.id));}} className="text-red-300 hover:text-red-500 transition-all"><Trash2 size={18}/></button></td></tr>))}</tbody></table></div></div></div>
    );
  } 
  ...
  // --- AUDITORIA ---
    ...
    mainContent = (
      <div className="space-y-8 animate-in fade-in"><h1 className="text-2xl font-black italic text-slate-800 tracking-tighter flex items-center gap-3"><History size={28} className="text-blue-600"/> Log Profissional</h1><div className="bg-white rounded-[40px] border shadow-sm overflow-hidden"><table className="w-full text-left font-bold italic text-xs"><thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400 border-b"><tr><th className="p-6">Data</th><th className="p-6">Usuário</th><th className="p-6">Ação</th><th className="p-6">Detalhes</th></tr></thead><tbody>{auditLogs.map((log) => (<tr key={log.id} className="border-t"><td className="p-6 text-[10px] text-slate-400 font-black">{log.date}</td><td className="p-6 text-xs text-blue-600 font-black uppercase">{log.user}</td><td className="p-6"><span className="bg-blue-100 px-3 py-1 rounded-full text-[8px] font-black uppercase">{log.action}</span></td><td className="p-6 text-xs text-slate-600">{log.details}</td></tr>))}</tbody></table></div></div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-16 bg-white border-b flex items-center justify-between px-8 shrink-0">
          <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest italic tracking-widest">INEPAD Governança Profissional</span>
