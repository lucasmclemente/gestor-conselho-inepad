...
const App = () => {
  ...
  // --- LÓGICA DE DADOS (FILTRANDO CONCLUÍDOS) ---
  const stats = useMemo(() => {
    ...
    const pendingList = allA.filter(a => a.status !== 'Concluído'); // FILTRO RIGOROSO
  ...
  // --- RENDERIZAÇÃO DE TABELAS (ISOLADAS PARA EVITAR ERRO) ---
  const AcoesRows = ({ limit }: { limit?: number }) => {
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-[#f8fafc]">
          ...
          {activeMenu === 'plano-geral' && (
            <div className="space-y-6 animate-in fade-in">
              ...
              <div className="bg-white p-8 rounded-[40px] border shadow-lg overflow-hidden">
                <h2 className="text-xl font-black italic text-slate-800 mb-6 flex items-center gap-2"><Target className="text-blue-600"/> Plano de Ações Completo (Somente Pendências)</h2>
                <table className="w-full text-left font-bold italic"><thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400 border-b italic"><tr><th className="p-4">Ação</th><th className="p-4">Reunião</th><th className="p-4 text-center">Status</th><th className="p-4">Dono</th></tr></thead><tbody><AcoesRows /></tbody></table>
              </div>
            </div>
          )}
          ...
          {activeMenu === 'reunioes' && (view === 'list' ? (
            ...
            <div className="animate-in fade-in pb-20">
              ...
              {tab === 'delib' && (
                <div className="space-y-6 animate-in fade-in">
                  <div className="bg-white p-8 rounded-[40px] border shadow-sm space-y-4"><h3 className="text-[10px] font-black uppercase italic text-slate-400">Votação</h3><input placeholder="Item para Decisão..." className="w-full p-4 border rounded-2xl text-sm font-black italic outline-none bg-slate-50/50" onChange={e => setNewDelib({title: e.target.value})} /><button onClick={()=>{if(newDelib.title){setCurrentMeeting({...currentMeeting, deliberacoes:[...currentMeeting.deliberacoes, {id: Date.now(), title: newDelib.title, votes: currentMeeting.participants.map(p=>({name:p.name, status:'Pendente', saved:false}))}]});}}} className="bg-blue-600 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase shadow-xl">Abrir Painel</button></div>
