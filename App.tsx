...
import { 
  ...
  Mail, UserCog, Settings, Camera, UserCircle, History, Filter
} from 'lucide-react';
...
const App = () => {
  ...
  const [meetings, setMeetings] = useState<any[]>([
    { 
      id: 1, title: 'Conselho Admin - Planejamento Q1', status: 'Concluída', date: '2026-01-10', time: '10:00', type: 'Presencial',
      participants: [{name: 'Membro Exemplo', email: 'membro@inepad.com.br'}],
      pautas: [{title: 'Aprovação de Budget', dur: '30', resp: 'Membro Exemplo'}],
      materiais: [], deliberacoes: [], acoes: [{id: 401, title: 'Revisão Contratual', status: 'Em andamento', resp: 'Membro Exemplo', date: '2026-03-01'}], atas: []
    }
  ]);
  ...
  // --- LOGS ---
  const [auditLogs, setAuditLogs] = useState<any[]>([{ id: 1, date: '19/02/2026 14:40', user: 'Sistema', action: 'Filtro', details: 'Filtro por reunião adicionado ao Dashboard.' }]);
  ...
  const updateActionStatus = (meetingId: number, actionId: number, newStatus: string) => {
    ...
    if (currentMeeting.id === meetingId) {
      setCurrentMeeting((prev:any) => ({ ...prev, acoes: prev.acoes.map((a:any) => a.id === actionId ? { ...a, status: newStatus } : a) }));
    }
    addLog('Status', `Ação #${actionId} -> ${newStatus}`);
  };

  // --- LÓGICA FILTRADA DO DASHBOARD ---
  const stats = useMemo(() => {
    const today = new Date(); today.setHours(0,0,0,0);
    const filteredMeetings = dashboardFilter === 'all' ? meetings : meetings.filter(m => m.id.toString() === dashboardFilter);
    const allA = filteredMeetings.flatMap(m => (m.acoes || []).map((a:any) => ({ ...a, mTitle: m.title, mId: m.id })));
    
    const statusCount = (st: string) => allA.filter(a => a.status === st).length;
    const atrasadas = allA.filter(a => a.status !== 'Concluída' && a.date && new Date(a.date) < today).length;

    return {
      concluida: `${statusCount('Concluída')}/${allA.length || 0}`,
      delibs: filteredMeetings.flatMap(m => m.deliberacoes || []).length,
      atas: filteredMeetings.reduce((acc, m) => acc + (m.atas?.length || 0), 0),
      atrasadas,
      allActions: allA,
      pieData: [
        { name: 'Em Andamento', value: statusCount('Em andamento'), color: '#3b82f6' },
        { name: 'Pendente', value: statusCount('Pendente'), color: '#f59e0b' },
        { name: 'Em Atraso', value: atrasadas, color: '#ef4444' }
      ],
      barData: filteredMeetings.map(m => ({
        name: m.date || 'S/D',
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-white rounded-[40px] shadow-2xl p-10">
          ...
          <form className="space-y-4" onSubmit={(e)=>{e.preventDefault(); const u=users.find(x=>x.email===authForm.email); if(u){setCurrentUser(u); setActiveMenu('dashboard');} else alert('Acesso negado');}}>
            <input type="email" placeholder="E-mail" className="w-full p-4 border rounded-2xl outline-none font-bold bg-slate-50" onChange={e=>setAuthForm({...authForm, email:e.target.value})} />
            <input type="password" placeholder="Senha" className="w-full p-4 border rounded-2xl outline-none bg-slate-50" />
  ...
  if (activeMenu === 'dashboard') {
      ...
      <div className="space-y-8 animate-in fade-in">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-2xl border shadow-sm">
            ...
            <select className="text-xs font-bold outline-none bg-transparent" value={dashboardFilter} onChange={e=>setDashboardFilter(e.target.value)}>
    ...
    mainContent = (
      <div className="space-y-6 animate-in fade-in"><button onClick={() => setActiveMenu('dashboard')} className="text-slate-400 flex items-center gap-1 text-[10px] font-black uppercase italic"><ChevronRight className="rotate-180" size={16}/> Voltar</button><div className="bg-white p-8 rounded-[40px] border shadow-lg overflow-hidden"><h2 className="text-xl font-black italic mb-6">Plano de Ações Integral</h2><table className="w-full text-left font-bold italic text-xs"><thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400 border-b"><tr><th className="p-6">Ação</th><th className="p-6 text-center">Status</th><th className="p-6">Responsável</th></tr></thead>
            <tbody>{stats.allActions.map((a:any, i:number)=>(<tr key={i} className="border-t hover:bg-slate-50"><td className="p-6 text-slate-800 font-bold">{a.title}</td><td className="p-6 text-center"><select value={a.status} className="p-2 border rounded-xl font-black uppercase text-[9px] outline-none" onChange={(e) => updateActionStatus(a.mId, a.id, e.target.value)}>{['Pendente','Em andamento','Concluída'].map(st => <option key={st} value={st}>{st}</option>)}</select></td><td className="p-6 font-black text-slate-600 uppercase">{a.resp}</td></tr>))}</tbody></table></div></div>
    );
  } else if (activeMenu === 'reunioes') {
    mainContent = view === 'list' ? (
      <div className="space-y-6 animate-in fade-in">
        <div className="flex justify-between items-center"><h1 className="text-2xl font-black italic tracking-tighter">Histórico</h1><button onClick={()=>{setCurrentMeeting({id:0,title:'',status:'Agendada',participants:[],pautas:[],materiais:[],deliberacoes:[],acoes:[],atas:[]}); setView('details'); setTab('info');}} className="bg-blue-600 text-white px-6 py-2.5 rounded-2xl font-black text-xs uppercase shadow-xl hover:scale-105 transition-all">+ Nova Reunião</button></div>
        <div className="grid gap-3">{meetings.map((m, idx) => (<div key={idx} onClick={()=>{setCurrentMeeting(m); setView('details'); setTab('info');}} className="bg-white p-6 rounded-[32px] border flex justify-between items-center group cursor-pointer hover:border-blue-400 shadow-sm transition-all"><div className="flex items-center gap-4"><div className="p-3 bg-slate-50 text-slate-400 rounded-2xl group-hover:bg-blue-600 group-hover:text-white transition-all"><Calendar size={20}/></div><div><h3 className="font-bold italic">{m.title}</h3><p className="text-[9px] font-black text-blue-600 uppercase italic">{m.status} • {m.date}</p></div></div><div className="flex items-center gap-4">{isAdm && <button onClick={(e)=>{e.stopPropagation(); if(window.confirm('Excluir?')){setMeetings(meetings.filter(x=>x.id!==m.id)); addLog('Exclusão', m.title);}}} className="p-2 text-slate-200 hover:text-red-500 transition-all"><Trash2 size={18}/></button>}<ChevronRight size={18}/></div></div>))}</div>
      </div>
    ) : (
      <div className="animate-in fade-in pb-20">
        <div className="flex justify-between items-center mb-8"><button onClick={()=>setView('list')} className="text-slate-400 flex items-center gap-1 text-[10px] font-black uppercase italic transition-all"><ChevronRight className="rotate-180" size={16}/> Voltar</button><button onClick={()=>{ if(!currentMeeting.title) return alert("Título!"); setMeetings(currentMeeting.id === 0 ? [{...currentMeeting, id: Date.now()}, ...meetings] : meetings.map(m=>m.id===currentMeeting.id?currentMeeting:m)); setView('list'); }} className="bg-slate-900 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase flex items-center gap-2"><Save size={16}/> Salvar</button></div>
          ...
          <div className="space-y-6 animate-in fade-in">
            ...
            {(currentMeeting.deliberacoes || []).map((d:any, i:any) => (
              <div key={i} className="bg-white rounded-[40px] border shadow-lg overflow-hidden mb-4"><div className="p-6 bg-slate-900 text-white flex justify-between items-center font-black italic text-sm"><span>{d.title}</span></div><div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6 bg-white">{d.votes.map((v:any, idx:number) => (
                <div key={idx} className={`p-6 rounded-[32px] border-2 ${v.saved ? 'bg-slate-50 opacity-80' : 'bg-white shadow-sm border-slate-100'}`}><div className="flex justify-between mb-4 font-black text-xs italic text-slate-700 uppercase"><span>{v.name}</span>{v.saved && <Lock size={12} className="text-green-500"/>}</div>
        ...
        {tab === 'acoes' && (
          <div className="bg-white p-8 rounded-[40px] border shadow-sm animate-in fade-in space-y-6"><h3 className="text-[10px] font-black uppercase italic text-slate-500">Plano de Ação</h3><div className="grid grid-cols-3 gap-4 p-6 bg-slate-50 rounded-[32px]"><input placeholder="Ação" className="col-span-3 p-3 text-xs border rounded-xl" value={tmpAcao.title} onChange={e=>setTmpAcao({...tmpAcao, title: e.target.value})} />
              <select className="p-3 text-xs border rounded-xl font-black uppercase italic bg-white" value={tmpAcao.resp} onChange={e=>setTmpAcao({...tmpAcao, resp: e.target.value})}>
                <option value="">Responsável</option>
                {(currentMeeting.participants || []).map((p:any, idx:number)=>(<option key={idx} value={p.name}>{p.name}</option>))}
              </select>
              <input type="date" className="p-3 text-xs border rounded-xl font-bold bg-white" value={tmpAcao.date} onChange={e=>setTmpAcao({...tmpAcao, date: e.target.value})} /><button onClick={()=>{if(tmpAcao.title){setCurrentMeeting({...currentMeeting, acoes: [...(currentMeeting.acoes || []), {...tmpAcao, id: Date.now()}]}); setTmpAcao({title:'', resp:'', date:'', status:'Pendente'});}}} className="bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase shadow-lg">Registrar</button></div><div className="overflow-x-auto"><table className="w-full text-left text-[10px] font-bold italic"><thead className="bg-slate-50 uppercase text-slate-400 border-b"><tr><th className="p-4">Tarefa</th><th className="p-4">Dono</th><th className="p-4 text-center">Status</th></tr></thead><tbody>{(currentMeeting.acoes || []).map((a:any, i:any) => (<tr key={i} className="border-t hover:bg-slate-50"><td className="p-4 text-slate-800 underline">{a.title}</td><td className="p-4">{a.resp}</td><td className="p-4 text-center"><select value={a.status} className="p-2 border rounded-xl font-black uppercase text-[8px]" onChange={(e)=>updateActionStatus(currentMeeting.id, a.id, e.target.value)}>{['Pendente', 'Em andamento', 'Concluída'].map(st => <option key={st} value={st}>{st}</option>)}</select></td></tr>))}</tbody></table></div></div>
        )}
        {tab === 'atas' && (
